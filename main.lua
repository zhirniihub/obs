...
function obfuscate(source,VarName,WaterMark)
        ...
        local Variable = "zhirnii_"
        local WM

        if source == nil then
            source = [[print("nigxger!")]]
        end

        local ticks = tick()

        if typeof(WaterMark) == "string" and WaterMark ~= nil then
            WM = "    "..tostring(WaterMark).." | zhirniihub"
        else
            WM = "    ".."WaterMark".." | zhirniihub"
        end
        ...
        local add_binary = function(number, s)
            ...
            local topic = {
                "GET YOUR ZHIR OUTTA HERE!!!",
                "zhirnii obfuscator on top nigga",
                "an psu/ironbrew fork? nope.... try again nigger",
                "boom nigga boom boom",
                "touch some grass",
                "bitch i shoot that gun, bitch i shoot that nigger",
                "this is my hijab and i won't remove",
                "your a fucking tranny, bitch you can't deny",
                "zhirdyai",
